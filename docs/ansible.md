...
**Two things are not yet true and will stop every play at the connection stage:**

1. The Pi's `authorized_keys` was written by cloud-init and contains only the *old*
   dev box's key. `ml5`'s `~/.ssh/id_ed25519.pub` has to be added to
   `bthek1@192.168.2.17:~/.ssh/authorized_keys`.
2. `ml5` holds a stale host key for the Pi, whose keys were regenerated by the
   reflash. Clear it: `ssh-keygen -f ~/.ssh/known_hosts -R 192.168.2.17`.
